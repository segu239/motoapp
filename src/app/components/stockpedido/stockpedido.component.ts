...
@Component({
  selector: 'app-stockpedido',
  templateUrl: './stockpedido.component.html',
  styleUrls: ['./stockpedido.component.css'],
  ...
  recibir() {
    ...
    if (this.operacionEnProceso) {
      Swal.fire({
        ...
        text: 'Ya hay una recepción en curso. Por favor espere.',
        icon: 'warning'
      });
      return;
    }
      ...
      error: (err) => {
        ...
        if (err.status === 409) {
          ...
          Swal.fire({
            ...
            text: err.error?.mensaje || 'Este pedido ya fue recibido anteriormente',
            icon: 'info'
          });
          ...
          Swal.fire({
            title: 'Error',
            text: 'Error al registrar la recepción. Por favor intente nuevamente.',
            icon: 'error'
          });
        }
      }
