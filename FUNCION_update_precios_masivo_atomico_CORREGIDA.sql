...
-- CORRECCIÓN CRÍTICA: Búsqueda de conflistas por tipomone (NO por marca)
...
BEGIN
    ...
    INSERT INTO cactualiza (
        ...
        1,
        COALESCE(p_tipo_modificacion, ''costo'') || '' + conflistas [CORREGIDA]'',
    ...
    FOR rec IN 
        SELECT 
            ...
            cod_iva,
            COALESCE(tipo_moneda, 1) as tipo_moneda
        FROM artsucursal 
    ...
    -- ===== RESPUESTA EXITOSA =====
    v_res := ''{"success":true,"message":"Actualización atómica CORREGIDA completada","registros_modificados":'' || v_cnt || 
             ...
             '',"con_margen_reparado":true,"prebsiva_actualizada":true,"busqueda_conflistas_corregida":true,"version":"CORREGIDA_20250815","timestamp":"'' || NOW() || ''"}'';
...
-- =====================================================
-- COMENTARIOS DE LA CORRECCIÓN CRÍTICA:
-- 
-- PROBLEMA IDENTIFICADO Y SOLUCIONADO:
-- - La función original buscaba conflistas por cod_marca (INCORRECTO)
-- - Debía buscar por tipomone que corresponde al tipo_moneda del artículo
-- 
-- CAMBIOS CRÍTICOS IMPLEMENTADOS:
-- 1. ✅ Añadido campo 'tipo_moneda' al SELECT principal
-- 2. ✅ Creado array v_tipos_moneda_modificados para rastrear tipos únicos
-- 3. ✅ CORREGIDA búsqueda de conflistas: 
--       FROM: WHERE TRIM(cl.cod_marca) = TRIM(p_marca)
--       TO:   WHERE cl.tipomone = ANY(v_tipos_moneda_modificados)
-- 4. ✅ Validación EXISTS corregida para verificar tipo_moneda
-- 5. ✅ Mantiene toda la lógica reparada de precios y prebsiva
-- 
-- CASOS DE PRUEBA CORREGIDOS:
-- - Artículo 8836: tipo_moneda=2, cod_iva=1 (IVA 21%)
-- - Debería procesar conflistas con tipomone=2 (IDs: 5, 6, 18)
-- - Resultado esperado: conflistas_actualizadas ≥ 3
--
-- VERIFICACIÓN CRÍTICA:
-- Antes: 0 conflistas procesadas (búsqueda por marca incorrecta)
-- Después: N conflistas procesadas (búsqueda por tipomone correcta)
-- =====================================================