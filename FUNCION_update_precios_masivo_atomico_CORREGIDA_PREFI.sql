-- =====================================================
-- FUNCIÓN CORREGIDA FINAL: update_precios_masivo_atomico
-- FECHA: 16 de Agosto de 2025
-- CORRECCIÓN CRÍTICA: Recálculo de prefi1-4 + Eliminación modificación conf_lista
-- PROBLEMA RESUELTO: prefi1-4 no se actualizaban + conf_lista se modificaba incorrectamente
-- VERSIÓN: CORREGIDA_PREFI_20250816
-- =====================================================
...
BEGIN
    ...
    INSERT INTO cactualiza (
        ...
        1,
        COALESCE(p_tipo_modificacion, ''costo'') || '' + prefi1-4 [CORREGIDA PREFI]'',
    ...
    -- ===== PROCESAMIENTO MASIVO CON MARGEN (CORREGIDO) =====
    FOR rec IN 
        SELECT 
    ...
    LOOP
        ...
        -- ===== CRÍTICO: RECALCULAR PREFI1-4 USANDO CONFIGURACIÓN ACTUAL DE CONF_LISTA =====
        UPDATE artsucursal a SET 
            prefi1 = CASE 
                WHEN EXISTS (SELECT 1 FROM conf_lista cl WHERE cl.listap = 1 AND cl.activa = true AND cl.tipomone = a.tipo_moneda) 
                THEN ROUND(a.precon * (1 + (SELECT CASE 
                    WHEN (SELECT alicuota1 FROM artiva WHERE cod_iva = a.cod_iva LIMIT 1) = 21.00 
                    THEN cl.preciof21 
                    ELSE cl.preciof105 
                END / 100.0 
                FROM conf_lista cl 
            ...
            END,
            prefi2 = CASE 
                WHEN EXISTS (SELECT 1 FROM conf_lista cl WHERE cl.listap = 2 AND cl.activa = true AND cl.tipomone = a.tipo_moneda) 
                THEN ROUND(a.precon * (1 + (SELECT CASE 
                    WHEN (SELECT alicuota1 FROM artiva WHERE cod_iva = a.cod_iva LIMIT 1) = 21.00 
                    THEN cl.preciof21 
                    ELSE cl.preciof105 
            ...
            END,
            prefi3 = CASE 
                WHEN EXISTS (SELECT 1 FROM conf_lista cl WHERE cl.listap = 3 AND cl.activa = true AND cl.tipomone = a.tipo_moneda) 
                THEN ROUND(a.precon * (1 + (SELECT CASE 
                    WHEN (SELECT alicuota1 FROM artiva WHERE cod_iva = a.cod_iva LIMIT 1) = 21.00 
                    THEN cl.preciof21 
                    ELSE cl.preciof105 
            ...
            END,
            prefi4 = CASE 
                WHEN EXISTS (SELECT 1 FROM conf_lista cl WHERE cl.listap = 4 AND cl.activa = true AND cl.tipomone = a.tipo_moneda) 
                THEN ROUND(a.precon * (1 + (SELECT CASE 
                    WHEN (SELECT alicuota1 FROM artiva WHERE cod_iva = a.cod_iva LIMIT 1) = 21.00 
                    THEN cl.preciof21 
                    ELSE cl.preciof105 
    ...
    -- ===== ELIMINADA SECCIÓN INCORRECTA DE MODIFICACIÓN DE CONF_LISTA =====
    -- ❌ REMOVIDO: Las líneas que modificaban preciof21 y preciof105 en conf_lista
    ...
    -- ===== RESPUESTA EXITOSA =====
    v_res := ''{"success":true,"message":"Actualización atómica CORREGIDA PREFI completada","registros_modificados":'' || v_cnt || 
             ...
             '',"con_margen_corregido":true,"prebsiva_actualizada":true,"prefi1_4_recalculados":true,"conf_lista_preservada":true,"version":"CORREGIDA_PREFI_20250816","timestamp":"'' || NOW() || ''"}'';
...
-- 
-- 1. ✅ RECÁLCULO COMPLETO DE PREFI1-4:
--    - Líneas 157-189: Implementación del recálculo usando conf_lista
--    - Aplica preciof21 para artículos con IVA 21%
--    - Aplica preciof105 para artículos con IVA 10.5%
-- 
-- 2. ✅ ELIMINACIÓN DE MODIFICACIÓN INCORRECTA:
--    - REMOVIDAS: Líneas que modificaban conf_lista.preciof21/preciof105
--    - PRESERVADA: Configuración original de políticas de precios
-- 
-- 3. ✅ MANTIENE TODAS LAS CORRECCIONES ANTERIORES:
--    - Cálculo correcto de precios con margen
--    - Conversión explícita de tipos (sin error array_append)
--    - Actualización de prebsiva
-- 
-- 4. ✅ IDENTIFICADORES ÚNICOS:
--    - Tipo incluye "+ prefi1-4 [CORREGIDA PREFI]"
--    - Versión: "CORREGIDA_PREFI_20250816"
--    - Campos: "prefi1_4_recalculados":true, "conf_lista_preservada":true
-- 
-- VALIDACIÓN ESPERADA - ARTÍCULO 9805 (+10%):
-- - precostosi: $3.5868 → $3.9455
-- - precon: $7.6000 → $8.3600  
-- - prefi1: $6.4558 → $6.9806 (usando preciof21=-16.50%)
-- - prefi2: $7.9748 → $8.8198 (usando preciof21=5.50%)
-- - prefi3: $5.3165 → $5.5398 (usando preciof21=-33.00%)
-- =====================================================